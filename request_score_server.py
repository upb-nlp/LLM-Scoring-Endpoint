"""Send the scoring scenarios to the LLM-Scoring-Endpoint server.

Usage:
    python request_score_server.py                       # hit the public endpoint
    python request_score_server.py --url http://localhost:5001
    python request_score_server.py --only paraphrasing   # filter by task
    python request_score_server.py --name paraphrasing_garbage  # run a single scenario
...
def send_score_request(base_url, task, data, timeout):
    url = f"{base_url.rstrip('/')}/score/{task}"
...
def print_scenario(scenario, scores, elapsed):
    name = scenario["name"]
    task = scenario["task"]
    student = scenario["data"]["student_response"]
    print(f"\n=== {name} ({task}) — {elapsed:.2f}s ===")
...
def main():
    parser = argparse.ArgumentParser(description="POST scoring scenarios to the LLM-Scoring-Endpoint server.")
    ...
    for scenario in scenarios:
        t0 = time.time()
        try:
            scores = send_score_request(args.url, scenario["task"], scenario["data"], args.timeout)
            ...
            continue
        elapsed = time.time() - t0
        print_scenario(scenario, scores, elapsed)
        n_ok += 1
